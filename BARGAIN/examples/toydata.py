...
class OpenAIProxy(Proxy):
    ...
    def proxy_func(self, data_record):
        ...
        if self.is_binary:
            ...
            #     data_record, ['World', 'Sports', 'Sci/Tech', 'Business'])
...
# here is the list of potential animals: ["lion", "tiger", "elephant", "giraffe", "zebra", "kangaroo", "panda", "koala", "dolphin", "whale", "eagle", "falcon", "bear", "wolf", "fox", "rabbit", "deer", "monkey", "hippopotamus", "rhinoceros"]
...
Here are the potential categories: ['World', 'Sports', 'Sci/Tech', 'Business']
You must choose Only ONE CATEGORY out of the list, and return THAT CATEGORY ONLY.
-----
here is the Article: {}
'''
...
proxy = OpenAIProxy(task, model='gpt-4.1-nano')
...
df = pd.read_csv("BARGAIN/examples/newtests/4news.csv").head(250)
...
bargain = BARGAIN_A(proxy, oracle, target=0.95,  delta=0.25, seed=0)


df['output'] = bargain.process(
    (df['Title'] + df['Description']).to_numpy())
...
df['is_correct'] = (
    df['injected_animals'].astype(str).str.replace(" ", "").str.lower()
    == df['output'].astype(str).str.replace(" ", "").str.lower()
)
