...
output, used_oracle = bargain.process(
    df['opinion_text'].to_numpy(), return_oracle_usage=True)
df['output'] = output
...
df.loc[~df['used_oracle'], 'output_oracle'] = oracle.get_pred(
    df.loc[~df['used_oracle'], 'opinion_text'].to_numpy())
df['is_correct'] = df['output_oracle'] == df['output']
print(
    f"Accuracy: {df['is_correct'].mean()}, Used Proxy: {used_proxy_count:.2f}")
